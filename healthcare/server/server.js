...
const HealthcareContract = require("./contracts/Healthcare.json")
const glob = require('glob')

const web3 = new Web3(new Web3.providers.HttpProvider("http://127.0.0.1:7545"))

let healthcare_contract = null
...
app.get('/DownloadMedicalEvidence/:user/:patient/:document', async (req, res) => {
  ...
  try {
    const doctorPermissions = await healthcare_contract.methods.getDoctorsPermissions(userAddress).call();
    ...
    const documents = await healthcare_contract.methods.getDocuments(patientAddress).call();
...
app.listen(port, async () => {
  ...
  healthcare_contract = new web3.eth.Contract(
